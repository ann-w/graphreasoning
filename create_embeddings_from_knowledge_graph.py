...
def generate_node_embeddings(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_node = {
            executor.submit(embedding_generator.generate_embedding, str(node)): node
            for node in G.nodes()
        }
        for i, future in enumerate(
            tqdm(as_completed(future_to_node), total=len(future_to_node))
        ):
            ...
            if (i + 1) % checkpoint_interval == 0:
                ...
                checkpoint_path = os.path.join(
                    checkpoint_dir, f"embeddings_checkpoint_{timestamp}.pkl"
                )
