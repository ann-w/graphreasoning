...
class ConversationAgent:
    ...
    def reply(self, interlocutor_reply: str | None = None) -> str:
        ...
        with assistant():
            curr_model += gen(
                name="response", max_tokens=1024, temperature=self.temperature
            )
...
def conversation_simulator(
    ...
    question_gpt,
    question_gpt_name="Engineer",
    answer_gpt_name="Biologist",
        ...
        if first:
            ...
            if only_last:
                ...
                conv = get_entire_conversation(
                    ...
                    conversation_turns,
                    marker_ch=marker_ch,
                    start_with_q=start_with_q,
                    question_gpt_name=question_gpt_name,
                )
...
def read_and_summarize(
    gpt,
    txt="This is a conversation.",
    q="",
):
    with system():
        lm = (
            gpt
            + "You analyze text and provide an accurate account of the content from all sides discussed."
        )
...
Keep your answers brief, and always challenge statements in a provokative way. As a creative individual, you inject ideas from other fields. """,
    ...
    bot_1 = ConversationAgent(
        chat_model=gpt,
        name=bot_name_1,
        instructions=bot_instructions_1,
        context_turns=total_turns,
        temperature=conv_temperature,
    )

    conversation_turns = conversation_simulator(
        bot_1,
        question_gpt=gpt_question_asker,
        question_gpt_name=bot_name_2,
        answer_gpt_name=bot_name_1,
...
def get_answer_LlamaIndex(
    llm,  # model, tokenizer,
    q,
    system_prompt="You are an expert in materials science.",
    chat_engine=None,
...
class ConversationAgent_LlamaIndex:
    def __init__(
        ...
        llm,
        name: str,
        instructions: str,  # context_turns: int = 2,
        index=None,
        chat_token_limit=2500,
        verbose=False,
        chat_mode="context",
        ...
        if index != None:
            print(
                f"Set up chat engine, with index, verbose={verbose}, chat_mode={chat_mode}."
            )
            ...
            self.chat_engine = SimpleChatEngine.from_defaults(
                llm=llm, system_prompt=self._instructions
            )
...
def conversation_simulator_LlamaIndex(
    ...
    question_gpt_name="Engineer",
    answer_gpt_name="Biologist",
    answer_instructions="You answer correctly.",
    ...
    include_N_turns_in_question_development=9999,
    single_shot_question=True,
    ...
    answer_agent = ConversationAgent_LlamaIndex(
        llm_answer,
        name=answer_gpt_name,
    ...
    for _ in range(total_turns):
        ...
        if only_last:
            ...
            conv = get_entire_conversation_LlamaIndex(
                q,
                conversation_turns[-NN:],
                marker_ch=marker_ch,
                start_with_q=start_with_q,
                question_gpt_name=question_gpt_name,
            )
...
The single question is:"""
            ...
            q_new, q_chat = get_answer_LlamaIndex(
                llm_question,  # model, tokenizer,
                ...
                + "You MUST respond with ONE new probing question. ONLY provide the question.",
            )
            ...
            summary_for_q, chat_engine = get_answer_LlamaIndex(
                llm_question,
                q=q,  # messages_to_prompt=messages_to_prompt,
                system_prompt="You analyze text and develop questions.",
                chat_engine=None,
            )
            q = f"""{marker_ch_outer}Please generate a thoughtful and challenging follow-up question. {sample_question}\n\nThe question is:"""
            print(f"\n\n### {question_gpt_name}: ", end="")
            q_new, chat_engine = get_answer_LlamaIndex(
                llm_question,
                q=q,  # messages_to_prompt=messages_to_prompt,
                system_prompt="You analyze text and develop questions.",
        ...
        if iterate_on_question:
            ...
            q_new, _ = get_answer_LlamaIndex(
                ...
                chat_mode=chat_mode,
                chat_token_limit=chat_token_limit,
                system_prompt="You pose questions.",
                chat_engine=q_chat,
            )
...
Keep your answers brief, and always challenge statements in a provokative way. As a creative individual, you inject ideas from other fields. """,
    ...
    chat_token_limit=2500,
    completion_to_prompt=None,
    iterate_on_question=False,
    iterate_on_question_with_earlier_context=True,
    verbose=False,
):
    (
        conversation_turns,
        ...
        response,
        answer_agent,
    ) = conversation_simulator_LlamaIndex(
        llm_answer,
        llm_question,  #  model, tokenizer,
        question_gpt_name=bot_name_2,
        answer_gpt_name=bot_name_1,
        ...
        data_dir=save_dir,
        marker_ch=marker_ch,
        marker_ch_outer=marker_ch_outer,
        start_with_q=start_with_q,
        only_last=only_last,
        ...
        chat_mode=chat_mode,
        chat_token_limit=chat_token_limit,
        iterate_on_question=iterate_on_question,
        iterate_on_question_with_earlier_context=iterate_on_question_with_earlier_context,
    )
    ...
    summary, bullet, keytakaway = read_and_summarize_LlamaIndex(
        llm_summarize,  # model, tokenizer ,
        txt,
        q=q,
    )
...
def get_entire_conversation_LlamaIndex(
    q,
    conversation_turns,
    marker_ch="### ",
    start_with_q=False,
