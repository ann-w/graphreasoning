...
def df2Graph_parallel(
    dataframe: pd.DataFrame,
    generate,
    repeat_refine=0,
    verbatim=False,
    output_folder="knowledge_graph_paper_examples",
) -> list:
    """
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(future_to_index):
            ...
            if count % checkpoint_interval == 0:
                checkpoint_filename = (
                    f"{checkpoint_dir}/triplets_checkpoint_{count}.json"
                )
...
def make_graph_from_text(
    ...
    if regenerate:
        concepts_list = df2Graph_parallel(
            df,
            generate,
            repeat_refine=repeat_refine,
            verbatim=verbatim,
            output_folder=data_dir,
        )  # changed this for parallel processing
...
def add_new_subgraph_from_text(
    ...
    try:
        ...
        if (
            graph_GraphML_to_add == None and G_newlymade == None
